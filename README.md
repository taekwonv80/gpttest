...
네이버 검색광고 성과를 자동 수집·분석하고 데일리 리포트로 전달하기 위한 Python 데이터 앱 MVP입니다. Streamlit과 Plotly로 작성하고, stlite가 Python을 WebAssembly로 브라우저에서 실행하므로 별도 서버 없이 GitHub Pages에서 열립니다.

## 현재 구현 범위

- 캠페인 분류: 플레이스 검색광고, 지역소상공인 광고, 파워링크
- 광고 지표: 광고비, 노출수, 클릭수, 클릭률, 평균 CPC
- 최근 5개 주차 선택, 일별 성과 추이, 캠페인 비교, 데일리 리포트 미리보기
- Plotly 클릭 추이, 효율 맵, 광고비 도넛, 5주 클릭률 차트
- 네이버 검색광고 API, Slack, 예약/POS, 콜 트래킹 연동 설계 화면
- GitHub Pages 배포

현재 공개 화면에는 **샘플 데이터만** 들어 있습니다. API 키를 브라우저 코드에 저장하지 않습니다. stlite는 UI를 서버 없이 실행하기 위한 구성이고, 실제 네이버 API 자동 수집은 GitHub Actions 또는 별도 안전한 실행 환경에서 처리해야 합니다.
...
stlite 로더는 CDN 모듈과 Python 파일을 불러오므로 로컬 서버로 확인합니다.
...
일반 Streamlit 서버로 실행하려면 의존성을 설치한 뒤 다음 명령을 사용합니다.
