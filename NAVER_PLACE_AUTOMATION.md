...
- `data/naver_place_daily.csv`: 5개 통계 탭에서 수집한 날짜별 누적 수치 및 전일 대비 증가분. 리포트의 플레이스 유입·예약/주문 신청·스마트콜·리뷰와 예약 상세는 횟수로, 플레이스 유입채널·유입키워드는 비율(%)로 누적됩니다.
- `data/naver_place_latest.json`: 마지막 수집 결과

로그인 만료나 화면 변경으로 실패하면 실행 페이지의 `naver-place-diagnostic` 파일에서 3일 동안 리포트·플레이스·스마트콜·예약주문·리뷰 탭별 진단 캡처를 받을 수 있습니다. 성공한 실행에서는 캡처를 업로드하지 않습니다. 로그인 세션이 만료되면 위 설정 스크립트를 다시 한 번 실행합니다.
