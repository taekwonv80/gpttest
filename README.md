...
4. 집계 데이터만 `data/*.json`에 저장한 뒤, 수집 성공 이벤트로 GitHub Pages를 자동 재배포합니다.
...
네이버 SearchAd API의 광고 통계와 로그인 세션 기반 스마트플레이스 통계를 자동 수집합니다. 스마트플레이스 수집 설정은 `NAVER_PLACE_AUTOMATION.md`를 참고하세요.

## 파워링크 액션 보드

`키워드 분석` 화면은 최근 7일 추세, 30일 기본 성과, 90일 장기 근거를 함께 사용해 각 등록 키워드와 실제 검색어를 `확대·유지·개선·감액·중지·제외`로 분류합니다.

- 광고 유형·추천 액션·검색어 필터
- 우선 처리 카드와 추천 근거·구체적인 변경 제안
- 최근 7일과 직전 7일 CTR 추세 비교
- 30일 표본 부족 시 90일 데이터로 보완하는 판단 신뢰도
- 등록 키워드는 90일 무노출일 때만 중지 후보로 분류
- 검토 대상 비용과 액션 분포
- 검토용 CSV 다운로드

추천은 광고 설정을 직접 변경하지 않습니다. 사람이 네이버 광고주센터에서 확인한 뒤 반영하는 승인형 운영을 전제로 합니다.
